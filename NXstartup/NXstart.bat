...
:: Determine which software versions are installed
...
for /f "usebackq tokens=* delims=" %%a in (`reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Siemens" 2^> nul`) do (
	...
	if defined __nx (
		if not defined NXCUSTOM_INSTALLED_NX_VERSIONS (
			...
			set NXCUSTOM_INSTALLED_NX_VERSIONS=!NXCUSTOM_INSTALLED_NX_VERSIONS! !__a:~-4!
		)
	)
...
if "%NXCUSTOM_LIBRARY_VERSIONS%" == "" (
	...
	call :report "and rename to NXxxxxlibrary where 'xxxx' is the version number."
	call :red_exit
)
...
set NXCUSTOM_AUTO_NX_VERSION=
set NXCUSTOM_AUTO_S3D_VERSION=
setlocal EnableDelayedExpansion
for %%a in (%NXCUSTOM_LIBRARY_VERSIONS%) do (
	for %%b in (%NXCUSTOM_INSTALLED_NX_VERSIONS%) do (
		if "%%a" == "%%b" (
			if not defined NXCUSTOM_VALID_NX_VERSIONS (
...
if not defined NXCUSTOM_VALID_NX_VERSIONS if not defined NXCUSTOM_VALID_S3D_VERSIONS (
	set NXCUSTOM_REPORT_VARS=TRUE
	call :report "No installed NX or Simcenter 3D version and NXcustom library version matches!"
	call :report "Installed NX versions: %NXCUSTOM_INSTALLED_NX_VERSIONS%"
	call :report "Installed Simcenter 3D versions: %NXCUSTOM_INSTALLED_S3D_VERSIONS%"
	call :report "Library versions: %NXCUSTOM_LIBRARY_VERSIONS%"
	call :red_exit
)
for %%a in (%NXCUSTOM_VALID_NX_VERSIONS%) do (
	if "%%a" geq "!NXCUSTOM_AUTO_NX_VERSION!" (
		set NXCUSTOM_AUTO_NX_VERSION=%%a
	)
)
...
endlocal & (
	...
	set NXCUSTOM_AUTO_NX_VERSION=%NXCUSTOM_AUTO_NX_VERSION%
...
:: UGII_BASE_DIR
if defined NX_LAUNCHER_INIT (
	call :validate_UGII_BASE_DIR
	for /f "usebackq" %%a in (`"%UGII_BASE_DIR%\NXBIN\env_print" NX_COMPATIBLE_BASE_RELEASE_VERSION`) do (
		set NXCUSTOM_APPLICATION_VERSION=%%~a
	)
	set NXCUSTOM_APPLICATION_NAME=NONE
) else (
	set UGII_BASE_DIR=
	...
	if not defined UGII_BASE_DIR (
		set NXCUSTOM_REPORT_VARS=TRUE
		call :report "No UGII_BASE_DIR key found in registry!"
		call :red_exit
	)
	call :validate_UGII_BASE_DIR
)
...
if exist "%NXCUSTOM_LIB%\NX_lib.version" (
	for /f "usebackq tokens=*" %%a in ("%NXCUSTOM_LIB%\NX_lib.version") do (set NXCUSTOM_LIB_VERSION=%%a)
)
...
if exist "%NXCUSTOM_START_DIR%\hooks\after_conf_eval\" (
	for /f "usebackq tokens=*" %%a in (`dir /b "%NXCUSTOM_START_DIR%\hooks\after_conf_eval\"`) do (
		call %NXCUSTOM_START_DIR%\hooks\after_conf_eval\%%a
	)
)
...
setlocal EnableDelayedExpansion
for /f "usebackq tokens=*" %%a in (`"%UGII_BASE_DIR%\NXBIN\env_print" -n`) do (
	set __a=%%a
	set NXCUSTOM_TITLE_VERSION=!__a:~3!
)
endlocal & (
	set NXCUSTOM_TITLE_VERSION=%NXCUSTOM_TITLE_VERSION%
)
...
for /f "usebackq tokens=1,2" %%a in (`"%UGII_BASE_DIR%\NXBIN\env_print" -n`) do (set NXCUSTOM_INSTALLED_VERSION=%%b)
...
if exist "%NXCUSTOM_START_DIR%\NXstart_customer.bat" (
	...
	call :report "Please move the script to NXstartup\hooks\after_report_vars\"
...
if defined NXCUSTOM_INSTALLED_VERSION	call :report "NX Full Version             = %NXCUSTOM_INSTALLED_VERSION%"
										call :report new_line
...
if exist "%NXCUSTOM_START_DIR%\hooks\after_report_vars\" (
	for /f "usebackq tokens=*" %%a in (`dir /b "%NXCUSTOM_START_DIR%\hooks\after_report_vars\"`) do (
		call %NXCUSTOM_START_DIR%\hooks\after_report_vars\%%a
	)
)
...
if exist "%NXCUSTOM_START_DIR%\hooks\after_application_start\" (
	for /f "usebackq tokens=*" %%a in (`dir /b "%NXCUSTOM_START_DIR%\hooks\after_application_start\"`) do (
		call %NXCUSTOM_START_DIR%\hooks\after_application_start\%%a
	)
)
...
set NXCUSTOM_VALID_PACKAGE_TITLE="NX-NX" "VIEW-NX Viewer" "NXCAM-NX" "MECHATRONICS-Mechatronics Concept Designer" "SECAM-NX" "LAYOUT-NX Layout" "SIMCENTER3D-Simcenter 3D" "SIMVIEWER-Simcenter 3D Results Viewer" "MOTION-NX" "NX2DEDIT-NX 2D Edit"
set NXCUSTOM_VALID_START_CONF_VARS=NXCUSTOM_APPLICATION_VERSION NXCUSTOM_APPLICATION_NAME NXCUSTOM_PACKAGE_NAME NXCUSTOM_LIBNAME NXCUSTOM_GROUP UGII_ROUTING_KIT_UNITS NXCUSTOM_SESSIONS NXCUSTOM_TEAMCENTER_ACTIVE START_DIR NXCUSTOM_TITLE SALT_LICENSE_SERVER SPLM_LICENSE_SERVER CDLMD_LICENSE_FILE NXCUSTOM_PASSED_ARGUMENTS NXCUSTOM_MANAGED_CUSTOMER_DEFAULTS NXCUSTOM_USER_SETTINGS_LOCATION NXCUSTOM_VERSION_IN_TITLE NXCUSTOM_LIBNAME_IN_TITLE NXCUSTOM_LOAD_OPTIONS_IN_USER_DIR UGII_UGDOC_BASE NXCUSTOM_REPORT_VARS NXCUSTOM_LOG_FILE NXCUSTOM_TEAMCENTER_ENV NXCUSTOM_USER_OVERRIDE_ARGS
...
set NXCUSTOM_VALID_TC_CONF_VARS=FMS_HOME FCC_CONFIG UGII_TCCS_ENV_NAME TCCS_CONFIG_HOME TC_ROOT TC_DATA UGII_UGMGR_COMMUNICATION UGII_UGMGR_HTTP_URL UGII_UGMGR_SOA_ENABLED UGII_UGMGR_USE_SOA NX_SSO_APP_ID NX_SSO_URL ENABLE_SSO UGII_CUSTOMER_ENV_NAME
...
set NXCUSTOM_USER_OVERRIDE_ARGS=/version /application /package /libname /group /units /sessions /managed /workdir /title /licserver /tcenv /passargs
exit /b
...
setlocal EnableDelayedExpansion
if "%arg%" == "START" (
	set NXCUSTOM_START_CONF=%NXCUSTOM_START_DIR%\NXstart.conf
	...
	if defined NXCUSTOM_TEAMCENTER_ENV (
		set NXCUSTOM_TC_CONF=%NXCUSTOM_START_DIR%\TCenv_%NXCUSTOM_TEAMCENTER_ENV%.conf
...
if /i "%NXCUSTOM_APPLICATION_NAME%" == "S3D" (
	...
	if not defined NXCUSTOM_APPLICATION_VERSION (
		set NXCUSTOM_APPLICATION_VERSION=%NXCUSTOM_AUTO_NX_VERSION%
	)
	for %%a in (%NXCUSTOM_VALID_NX_VERSIONS%) do (
		if "!NXCUSTOM_APPLICATION_VERSION!" == "%%a" (set valid=true)
	)
...
if not exist "%NXCUSTOM_PART_FILE%" (
	set abort=true
	set NXCUSTOM_REPORT_VARS=TRUE
